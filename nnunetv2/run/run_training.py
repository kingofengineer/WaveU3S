...
def run_ddp(rank, dataset_name_or_id, configuration, fold, model, tr, p, use_compressed, disable_checkpointing, c, val, pretrained_weights, npz, world_size):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, model, tr, p,
                                           use_compressed)
...
def run_training(dataset_name_or_id: Union[str, int],
                 configuration: str, fold: Union[int, str],model: str,
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, model, trainer_class_name,
...
def run_training_entry():
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.model, args.tr, args.p, args.pretrained_weights,
                 args.num_gpus, args.use_compressed, args.npz, args.c, args.val, args.disable_checkpointing,device=device)
