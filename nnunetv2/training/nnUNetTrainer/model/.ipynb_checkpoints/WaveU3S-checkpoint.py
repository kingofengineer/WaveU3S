...
sys.path.append('/home/jupyter-ztz/ZHONG_tianzhao/code/waveUNetr-main/unetr_plus_plus-main')
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.epa_block = W_EPA(input_size=input_size ,hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate)
...
class TransformerBlock_O(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = EPA_O(input_size=input_size ,hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads, channel_attn_drop=dropout_rate,spatial_attn_drop=dropout_rate)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class W_EPA(nn.Module):
    ...
    def forward(self, x, H, W, D):
        ...
        #([4, 32, 24, 24, 24])
        #print(x.shape)
        #output, LLH, LHL, LHH, HLL, HLH, HHL, HHH = self.dwt(self.reduce(x))
        ...
        x_idwt = self.idwt(x_dwt)#[B,C,H/2,W/2]([1, 64, 12, 12])
        ...
        v_CA = v_CA.transpose(-2, -1)
        v_SA = v_SA.transpose(-2, -1)#[4,4,8,13824]
        ...
        #print('q_c',q_c.shape)
        #print('k_CAtranspose',k_CA.transpose(-2, -1).shape)
...
class waveUnetr_pp_v8(SegmentationNetwork):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            feature_size: dimension of network feature size.网络中特征图的通道数，
        ...
        Examples::
            ...
            输入张量是单通道的（即黑白图像），输出张量是4个通道，表示分别对应14个类别的像素概率分布。每个输出通道的大小为(64, 128, 128)，
            输入数据的特征大小(feature size)为16，使用批归一化(batch norm)并且使用深度监督（deep supervision），在UNet层数上进行分割任务，
            其层数分别为[3,3,3,3]，每层输出通道数分别为[32,64,128,256]，使用4个注意力头(num_heads=4)。
        """
